...
public class TestDRH {
	...
	public void test3()  {
		Set<String> tmp =  Arrays.stream(drh.list_personnel()).map(emp->emp.getNom()).collect(Collectors.toSet());
	...
	public void test6(){
		String nom = employes.keySet().iterator().next();
		drh.tu_es_vire(nom);		
		assertFalse(Arrays.stream(drh.list_personnel()).anyMatch(e -> e.getNom().equals(nom)));
	}
}
